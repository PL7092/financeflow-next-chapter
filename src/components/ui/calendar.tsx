...
function Calendar({ className, classNames, showOutsideDays = true, ...props }: CalendarProps) {
  ...
  return (
    <DayPicker
      ...
      locale={locale}
      weekStartsOn={1}
      className={cn("p-3 pointer-events-auto w-fit max-w-sm", className)}
      classNames={{
        ...
        head_cell: "text-muted-foreground w-8 h-8 text-xs font-medium flex items-center justify-center",
        row: "grid grid-cols-7 gap-0",
        cell: "w-8 h-8 text-center text-sm p-0 relative flex items-center justify-center [&:has([aria-selected].day-range-end)]:rounded-r-md [&:has([aria-selected].day-outside)]:bg-accent/50 [&:has([aria-selected])]:bg-accent first:[&:has([aria-selected])]:rounded-l-md last:[&:has([aria-selected])]:rounded-r-md focus-within:relative focus-within:z-20",
